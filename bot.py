...
@bot.event
async def on_ready():
    activity = discord.Activity(type=discord.ActivityType.watching, name="!iqa help | Assalamu'Alaikum wa rahmatullahi wa barakatuh")
...
async def search(ctx, lang_code: str, search_query: str):
    ...
    embed=discord.Embed(title="View search results", url=url, color=discord.Colour(12443287))
...
async def answer(ctx, lang_code: str, question_id: int):
    ...
    if description != "...":
      embed=discord.Embed(title=title, url=url, description=description, color=discord.Colour(12443287))
      await ctx.send(embed=embed)
    else:
      embed=discord.Embed(title=title, url=url, description="Empty", color=discord.Colour(12443287))
      await ctx.send(embed=embed)
    ...
    if len(heads) == 1:
      ...
      if len(heads) <= 4:
        for tuple_ in heads[0:len(heads) - 1]:
          embed.add_field(name="\u200b", value="[{}]({})".format(tuple_[0], tuple_[1]), inline=False)
      else:
...
async def important_topics(ctx, lang_code: str = "en"):
    ...
    title = heads[-1]
    value = "[{}]({})\n\n[{}]({})\n\n[{}]({})\n\n[{}]({})".format(heads[0][0], heads[0][1], heads[1][0], heads[1][1], heads[2][0], heads[2][1], heads[3][0], heads[3][1])
    embed =discord.Embed(title=title, url=url, color=discord.Colour(12443287))
...
async def categories(ctx, lang_code: str, cat_type: str, category_id: int):
    ...
    url = "https://islamqa.info/{}/categories/{}/{}".format(lang_code, category_type, category_id)
    heads = await fetchCategories(url)
    title = heads[-1]
    embed =discord.Embed(title=title, url=url, color=discord.Colour(12443287))
    if len(heads) == 1:
